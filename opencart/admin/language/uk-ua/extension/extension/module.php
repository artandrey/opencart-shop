<?php
// Heading
$_['heading_title']    = 'Модулі';

// Text
$_['text_success']     = 'Налаштування змінені';
$_['text_layout']      = 'Після установки та налаштування модуля, якщо ви бажаєте відображати його на сторінках магазину, ви маєте додати модуль у розділі <a href="%s" class="alert-link">Дизайн - Макети</a>';
$_['text_add']         = 'Додати';
$_['text_list']        = 'Список модулів';
$_['text_hide_modules'] = 'У списку не показані модулі, які приховані в <a href="%s" class="alert-link">розділі управління правами користувачів</a>';

// Column
$_['column_name']      = 'Назва модуля';
$_['column_status']    = 'Статус';
$_['column_action']    = 'Дія';

// Entry
$_['entry_code']       = 'Модуль';
$_['entry_name']       = 'Назва модуля';

// Error
$_['error_permission'] = 'У вас недостатньо прав для внесення змін';
$_['error_name']       = 'Назва має містити від 3 до 64 символів';
$_['error_code_name']  = 'Код модуля має містити від 3 до 32 символів';
$_['error_code']       = 'Вкажіть модуль';